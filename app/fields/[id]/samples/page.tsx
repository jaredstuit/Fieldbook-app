...
import { getFieldContext, fmtDate } from '@/lib/field';
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { supabase, field, ranch, grower } = await getFieldContext(id);
  ...
  return (
    <AppShell active="Fields">
      <FieldHeader field={field} ranch={ranch} grower={grower} active="samples" />
      <section className="card">
        <div className="section-head">
          ...
          <Link className="btn" href={`/fields/${id}/samples/new`}>+ Add sample</Link>
        </div>
              ...
              <article className="sample-card" key={s.id}>
                <div className="sample-card-head">
                  ...
                  <div>
                    ...
                    <Link className="text-link" href={`/fields/${id}/samples/${s.id}/edit`}>Edit</Link>
                  </div>
                </div>
