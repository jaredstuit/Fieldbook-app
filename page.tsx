...
export default async function Home() {
  ...
  return <AppShell><div className="topbar"><div><h1>Growers</h1><p className="subtle">{organization.name} · live database</p></div><Link className="btn" href="/growers/new">+ Grower</Link></div>
    ...
    <div className="card">{rows.length?<table className="table"><thead><tr><th>Grower</th><th>Ranches</th><th>Fields</th><th>Acres</th></tr></thead><tbody>{rows.map((g:any)=><tr key={g.id}><td><Link href={`/growers/${g.id}`}><strong>{g.name}</strong></Link></td><td>{g.ranchCount}</td><td>{g.fieldCount}</td><td>{g.acres.toFixed(1)}</td></tr>)}</tbody></table>:<div className="empty"><h3>No growers yet</h3><p className="subtle">Create your first grower, then add ranches and fields beneath it.</p><Link className="btn" href="/growers/new">Create first grower</Link></div>}</div>
  </AppShell>;
}
